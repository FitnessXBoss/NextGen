...
public class UserSessionService
{
    ...
    public async Task<bool> LoadAdditionalUserDataAsync()
    {
        if (CurrentUser != null)
        {
            ...
            {
                using (var cmd = new NpgsqlCommand("SELECT u.user_id, u.username, u.last_login, e.employee_id, e.first_name, e.last_name, e.position, e.email, e.phone, e.address, e.photo_url, r.role_id, r.role_name, r.permissions FROM employees e JOIN users u ON e.employee_id = u.employee_id JOIN roles r ON e.role_id = r.role_id WHERE u.employee_id = @employeeId", conn))
                {
                    ...
                    using (var reader = await cmd.ExecuteReaderAsync())
                    {
                        if (reader.Read())
                        {
                            // Удостоверьтесь в правильности порядка и метода чтения данных:
